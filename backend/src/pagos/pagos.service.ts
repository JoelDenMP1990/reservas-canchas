...
export class PagosService {
  ...
  listar(): Promise<Pago[]> {
    return this.pagosRepository.find({ relations: ['reserva'] });
  }

  async obtenerPorId(id: string): Promise<Pago> {
    const pago = await this.pagosRepository.findOne({ where: { id }, relations: ['reserva'] });
    if (!pago) {
  ...
  // crear(): registra el pago de una reserva, lo procesa y confirma la reserva.
  async crear(dto: CrearPagoDto): Promise<Pago> {
    const reserva = await this.reservasRepository.findOneBy({ id: dto.reservaId });
    if (!reserva) {
      throw new NotFoundException('Reserva no encontrada');
    }
    const pagoExistente = await this.pagosRepository.findOne({ where: { reserva: { id: reserva.id } } });
    if (pagoExistente) {
